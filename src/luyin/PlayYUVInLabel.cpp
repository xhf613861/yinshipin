...
PlayYUVInLabel::PlayYUVInLabel(void *winid, QObject *parent) : QThread(parent), _winId(winid)
{
    connect(this, &PlayYUVInLabel::finished,
            this, &PlayYUVInLabel::deleteLater);
}

PlayYUVInLabel::~PlayYUVInLabel()
{
    requestInterruption();
    quit();
    wait();

    qDebug() << "PlayYUVInLabel xigou";
}

void PlayYUVInLabel::run()
{
    // 窗口
